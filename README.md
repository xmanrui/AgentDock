...
AgentDock is a local-first, project-scoped AI CLI session dashboard for JetBrains IDEs. It discovers Codex CLI, Claude Code, and Gemini CLI sessions associated with the current project and brings them into a full-height panel on the right side of the IDE, where developers can create, inspect, resume, pin, filter, and monitor ongoing work.
...
- A split new-session control beside search: click the current default to launch immediately, or choose any provider's normal/`YOLO` mode from the dropdown and make it the new default
...
- `Resume`, provider-specific `YOLO`, and `Pin / Unpin` actions on every session card
...
- Configurable provider executables and detection, normal/YOLO start, and normal/YOLO resume command templates
...
| Resume | Resumes the session with the provider's normal permission behavior |
...
| Provider | Executable | Detection command | Start command | YOLO start command | Resume command | YOLO resume command |
| --- | --- | --- | --- | --- | --- | --- |
| Codex | `codex` | `codex --version` | `{{executable}}` | `{{executable}} --dangerously-bypass-approvals-and-sandbox` | `{{executable}} resume {{providerSessionId?}}` | `{{executable}} resume --dangerously-bypass-approvals-and-sandbox {{providerSessionId?}}` |
| Claude Code | `claude` | `claude --version` | `{{executable}} --ide --name {{sessionName}}` | `{{executable}} --ide --name {{sessionName}} --dangerously-skip-permissions` | `{{executable}} --resume {{providerSessionId?}} --ide` | `{{executable}} --resume {{providerSessionId?}} --ide --dangerously-skip-permissions` |
| Gemini CLI | `gemini` | `gemini --version` | `{{executable}}` | `{{executable}} --yolo` | `{{executable}} --resume {{providerSessionId?}}` | `{{executable}} --resume {{providerSessionId?}} --yolo` |
...
The `YOLO` new-session and card actions bypass the selected CLI's normal permission checks and sandbox restrictions. Use them only in workspaces you trust.
...
1. Click the provider button to the right of `Search sessions` to create a session immediately with the current default provider and mode.
2. To use another option, open its chevron menu and click the provider name for normal mode or the `YOLO` button on the same row.
3. The selected provider and mode become the new application-wide default for the next direct launch.
4. AgentDock starts the CLI in the current project root, opens a Terminal tab, and automatically binds the temporary card to the provider's real session when it appears in local history.

#### Resume a Session

1. Click `Resume` to resume with the provider's normal permission behavior, or `YOLO` to use its configured permission-bypass command.
...
- New sessions can be created from the split control beside search; rename and archive controls are not exposed in the current dashboard.
...
AgentDock 是一个本地优先、面向 JetBrains IDE 的项目级 AI CLI 会话面板。它自动发现当前项目关联的 Codex CLI、Claude Code 和 Gemini CLI 会话，并集中展示在 IDE 右侧的全高面板中，方便开发者新建、查看、恢复、置顶、筛选和监控 AI 工作。
...
- 搜索框右侧提供分体式新会话入口：点击当前默认项可直接创建，也可从下拉菜单选择任意 Provider 的常规/`YOLO` 模式并将其设为新默认值
...
- 每张会话卡片提供 `Resume`、Provider 专属 `YOLO` 和 `Pin / Unpin`
- 卡片灰点/绿点表示 AgentDock 启动的对应 CLI 终端是否处于非活动/活动状态
- 终端标题区分空闲、工作中和等待查收三种状态
- 大模型工作时，在终端标题上方的指向式消息框中从右向左滚动展示经过过滤的流式输出
- Provider executable、detect、常规/YOLO start、常规/YOLO resume command template 可配置
...
| Provider | Executable | Detect command | Start command | YOLO start command | Resume command | YOLO resume command |
| --- | --- | --- | --- | --- | --- | --- |
| Codex | `codex` | `codex --version` | `{{executable}}` | `{{executable}} --dangerously-bypass-approvals-and-sandbox` | `{{executable}} resume {{providerSessionId?}}` | `{{executable}} resume --dangerously-bypass-approvals-and-sandbox {{providerSessionId?}}` |
| Claude Code | `claude` | `claude --version` | `{{executable}} --ide --name {{sessionName}}` | `{{executable}} --ide --name {{sessionName}} --dangerously-skip-permissions` | `{{executable}} --resume {{providerSessionId?}} --ide` | `{{executable}} --resume {{providerSessionId?}} --ide --dangerously-skip-permissions` |
| Gemini CLI | `gemini` | `gemini --version` | `{{executable}}` | `{{executable}} --yolo` | `{{executable}} --resume {{providerSessionId?}}` | `{{executable}} --resume {{providerSessionId?}} --yolo` |
...
新建会话入口和会话卡片上的 `YOLO` 操作都会跳过对应 CLI 的常规权限确认和沙箱限制，请仅在可信工作区中使用。
...
1. 点击 `Search sessions` 右侧的 Provider 主按钮，直接使用当前默认 Provider 和模式创建会话。
2. 如需切换，点击右侧箭头展开菜单；点击每行左侧 Provider 名称使用常规模式，点击同一行的 `YOLO` 使用 YOLO 模式。
3. 本次选择的 Provider 和模式会成为 application-level 新默认值，下一次可直接点击启动。
4. AgentDock 会在当前项目根目录启动 CLI、打开 Terminal Tab，并在本地历史出现真实会话后将临时卡片自动绑定到真实 Provider Session ID。

#### 恢复会话

1. 点击 `Resume` 按 Provider 的常规权限行为恢复，或点击 `YOLO` 使用已配置的跳过权限确认命令。
...
- 搜索框右侧的分体式入口可新建会话；重命名和归档控件暂未在当前 Dashboard 中开放。
